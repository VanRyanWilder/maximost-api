...
class BaseClient {
    ...
    async getToken(env) {
        if ((0, emulator_1.useEmulator)(env)) {
            return 'owner';
        }
    ...
    async fetch(apiSettings, requestData, env) {
        ...
        return await this.fetchWithRetry(fullUrl, {
            ...
            headers: {
                ...
                'User-Agent': `Code-Hex/firebase-auth-cloudflare-workers/${version_1.version}`,
                'X-Client-Version': `Code-Hex/firebase-auth-cloudflare-workers/${version_1.version}`,
    ...
    async fetchWithRetry(url, init, retryAttempts = 0) {
        try {
            ...
            try {
                ...
                throw new HttpError(res.status, text, {
                    cause: new errors_1.FirebaseAppError(errors_1.AppErrorCodes.UNABLE_TO_PARSE_RESPONSE, `Error while parsing response data: "${String(err)}". Raw server ` +
            ...
            if (err instanceof HttpError) {
                ...
                try {
                    ...
                    if (errorCode) {
                        throw errors_1.FirebaseAuthError.fromServerError(errorCode, json);
                    }
                }
                catch (err) {
                    if (err instanceof errors_1.FirebaseAuthError) {
                        throw err;
                    }
                }
                throw new errors_1.FirebaseAppError(errors_1.AppErrorCodes.INTERNAL_ERROR, `Error while sending request or reading response: "${err}". Raw server ` +
                    `response: Status code: "${err.status}". Outgoing ` +
                    `request: "${init.method} ${url}."`);
            }
            throw new errors_1.FirebaseAppError(errors_1.AppErrorCodes.NETWORK_ERROR, `Error while making request: ${String(err)}`);
        }
    }
    /**
    ...
    getErrorCode(response) {
        return ((0, validator_1.isNonNullObject)(response) && response.error && response.error.message) || null;
    }
