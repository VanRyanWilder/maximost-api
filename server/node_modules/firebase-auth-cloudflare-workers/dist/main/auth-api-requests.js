...
exports.AuthApiClient = exports.FIREBASE_AUTH_SET_CUSTOM_USER_CLAIMS = exports.FIREBASE_AUTH_REVOKE_REFRESH_TOKENS = exports.FIREBASE_AUTH_GET_ACCOUNT_INFO = exports.FIREBASE_AUTH_CREATE_SESSION_COOKIE = void 0;
 ...
 * @internal
 */
exports.FIREBASE_AUTH_CREATE_SESSION_COOKIE = new api_requests_1.ApiSettings('v1', ':createSessionCookie', 'POST')
    // Set request validator.
    .setRequestValidator((request) => {
    // Validate the ID token is a non-empty string.
    if (!(0, validator_1.isNonEmptyString)(request.idToken)) {
        throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.INVALID_ID_TOKEN);
    }
    // Validate the custom session cookie duration.
    if (!(0, validator_1.isNumber)(request.validDuration) ||
    ...
    if (!(0, validator_1.isNonEmptyString)(response.sessionCookie)) {
        throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.INTERNAL_ERROR);
    }
});
/**
 ...
 * @internal
 */
exports.FIREBASE_AUTH_GET_ACCOUNT_INFO = new api_requests_1.ApiSettings('v1', '/accounts:lookup', 'POST')
    // Set request validator.
    .setRequestValidator((request) => {
    if (!request.localId && !request.email && !request.phoneNumber && !request.federatedUserId) {
        throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.INTERNAL_ERROR, 'INTERNAL ASSERT FAILED: Server request is missing user identifier');
    }
})
    // Set response validator.
    .setResponseValidator((response) => {
    if (!response.users || !response.users.length) {
        throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.USER_NOT_FOUND);
    }
});
/**
 ...
 */
exports.FIREBASE_AUTH_REVOKE_REFRESH_TOKENS = new api_requests_1.ApiSettings('v1', '/accounts:update', 'POST')
    // Set request validator.
    .setRequestValidator((request) => {
    ...
    if (typeof request.validSince !== 'undefined' && !(0, validator_1.isNumber)(request.validSince)) {
        throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.INVALID_TOKENS_VALID_AFTER_TIME);
    }
})
    // Set response validator.
    .setResponseValidator((response) => {
 ...
 * @link https://github.com/firebase/firebase-admin-node/blob/9955bca47249301aa970679ae99fe01d54adf6a8/src/auth/auth-api-request.ts#L746
 */
exports.FIREBASE_AUTH_SET_CUSTOM_USER_CLAIMS = new api_requests_1.ApiSettings('v1', '/accounts:update', 'POST')
    // Set request validator.
    .setRequestValidator((request) => {
    // localId is a required parameter.
    if (typeof request.localId === 'undefined') {
    ...
    if (typeof request.customAttributes !== 'undefined') {
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                // this field.
                throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.INVALID_CLAIMS, error.message);
            }
            throw error;
        }
        const invalidClaims = [];
        ...
        if (invalidClaims.length > 0) {
            throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.FORBIDDEN_CLAIM, invalidClaims.length > 1
        ...
        if (request.customAttributes.length > MAX_CLAIMS_PAYLOAD_SIZE) {
            throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.CLAIMS_TOO_LARGE, `Developer claims payload should not exceed ${MAX_CLAIMS_PAYLOAD_SIZE} characters.`);
        }
    }
})
    // Set response validator.
...
class AuthApiClient extends client_1.BaseClient {
    /**
    ...
    async createSessionCookie(idToken, expiresIn, env) {
        ...
        const res = await this.fetch(exports.FIREBASE_AUTH_CREATE_SESSION_COOKIE, request, env);
    ...
    async getAccountInfoByUid(uid, env) {
        if (!(0, validator_1.isUid)(uid)) {
            throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.INVALID_UID);
        }
        const request = {
            localId: [uid],
        };
        const res = await this.fetch(exports.FIREBASE_AUTH_GET_ACCOUNT_INFO, request, env);
        // Returns the user record populated with server response.
        return new user_record_1.UserRecord(res.users[0]);
    }
    /**
    ...
    async revokeRefreshTokens(uid, env) {
        ...
        const res = await this.fetch(exports.FIREBASE_AUTH_REVOKE_REFRESH_TOKENS, request, env);
        return res.localId;
    }
    /**
    ...
    async setCustomUserClaims(uid, customUserClaims, env) {
        ...
        if (!(0, validator_1.isUid)(uid)) {
            ...
            throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.INVALID_ARGUMENT, 'CustomUserClaims argument must be an object or null.');
        }
        ...
        const res = await this.fetch(exports.FIREBASE_AUTH_SET_CUSTOM_USER_CLAIMS, request, env);
        return res.localId;
    }
}
