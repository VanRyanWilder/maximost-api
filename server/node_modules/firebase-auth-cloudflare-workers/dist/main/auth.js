...
class BaseAuth {
    ...
    async verifyIdToken(idToken, checkRevoked = false, env, clockSkewSeconds) {
        ...
        if (checkRevoked) {
            return await this.verifyDecodedJWTNotRevokedOrDisabled(decodedIdToken, errors_1.AuthClientErrorCode.ID_TOKEN_REVOKED, env);
        }
    ...
    async verifySessionCookie(sessionCookie, checkRevoked = false, env) {
        ...
        // Whether to check if the token was revoked.
        if (checkRevoked) {
            return await this.verifyDecodedJWTNotRevokedOrDisabled(decodedIdToken, errors_1.AuthClientErrorCode.SESSION_COOKIE_REVOKED, env);
        }
        return decodedIdToken;
    }
    /**
    ...
    async verifyDecodedJWTNotRevokedOrDisabled(decodedIdToken, revocationErrorInfo, env) {
        ...
        if (user.disabled) {
            throw new errors_1.FirebaseAuthError(errors_1.AuthClientErrorCode.USER_DISABLED, 'The user record is disabled.');
        }
        ...
        if (user.tokensValidAfterTime) {
            ...
            if (authTimeUtc < validSinceUtc) {
                throw new errors_1.FirebaseAuthError(revocationErrorInfo);
            }
        }
