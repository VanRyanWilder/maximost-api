...
export const authMiddleware = async (c: AuthContext, next: Next) => {
  ...
  try {
    ...
    if (!decodedPayload || typeof decodedPayload.user_id !== 'string') {
        throw new Error('Invalid token payload');
    }

    // 4. Attach the user payload to the context and proceed
    const userPayload: { user_id: string, email?: string } = { user_id: decodedPayload.user_id };
    if (decodedPayload.email) {
        userPayload.email = decodedPayload.email as string;
    }
    c.user = userPayload;
    await next();
